...
def recognize_plate(img, coords):
    try:
        ...
        plate_to_string = OCR()
        result = plate_to_string.ocr(box)

        #im = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
        #img = cv2.resize(im, dsize=(140, 70), interpolation=cv2.INTER_LINEAR)
        #img = img[np.newaxis, ..., np.newaxis] / 255.
        #img = tf.constant(img, dtype=tf.float32)
        #prediction = plate_to_string.predict_from_array(img).numpy()
        #probs = plate_to_string.probs_to_plate(prediction)
        ...
        if class_name not in allowed_classes:
            ...
            if read_plate:
                ...
                if plate_number != None:
                    cv2.putText(image, plate_number, (int(coor[0])-40, int(coor[1]-height_ratio)),
